...
if "sd.pkl" in os.listdir():
    ...
    for i in range(epochs):
        ...
        print(f"Epoch {i}, Loss {'%.2f'%(lsd / (x.shape[0] // BATCH_SIZE))}, Accu: {'%.4f'%accuracy(yhat.data, y.data)}")
    save_state_dict("sd.pkl",model)

yhat_val = model.forward(x_val)
print(f"Evaluation Accuracy: {(y_val.data == np.argmax(yhat_val.data,axis=1)).sum()/4000}")
