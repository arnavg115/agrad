    ...
    def step(self, module: Module)->None:
        for i in module.state_dict():
            if self.v_curr.get(i[0]) is None:
                self.v_curr[i[0]] = 0
            self.v_curr[i[0]] = self.beta * self.v_curr[i[0]] + (1-self.beta)* i[1].grad
    ...
    def step(self, module: Module)->None:
        for i in module.state_dict():
            if self.s_curr.get(i[0]) is None:
                self.s_curr[i[0]] = 0
            self.s_curr[i[0]] = self.beta * self.s_curr[i[0]]+ (1-self.beta) * np.power(i[1].grad, 2)
...
class adam(optim):
    ...
    def step(self, module: Module):
        for i in module.state_dict():
            ...
            if self.v_curr.get(i[0]) is None:
                self.v_curr[i[0]] = 0
            self.v_curr[i[0]] = self.beta2 * self.v_curr[i[0]] + (1-self.beta2)* i[1].grad
            self.s_curr[i[0]] = self.beta1 * self.s_curr[i[0]]+ (1-self.beta1) * np.power(i[1].grad, 2)
